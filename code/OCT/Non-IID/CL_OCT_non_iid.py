# =====================================================
# Centralized (normal) learning: ResNet18 on OCT
# ====================================================
import os
import torch
...
from data_processs import SkinData, dataset_iid, dataset_non_iid, DatasetSplit, dataset_split
...
# ===================================================================
program = "Central Learning ResNet18 on OCT"
program1 = "Central Learning ResNet18 on OCT_final"
print(f"---------{program}----------")
...
dict_users = dataset_split(train)
dict_users_test = dataset_split(test)

train_iterator = DataLoader(DatasetSplit(dataset_train,dict_users), shuffle=True, batch_size=64)
test_iterator = DataLoader(DatasetSplit(dataset_train,dict_users_test), shuffle=True, batch_size=256)
...
net_glob = ResNet18(BasicBlock, [2, 2, 2, 2], 4)  # Class labels for OCT =4
...
def train(model, device, iterator, optimizer, criterion):
    ...
    for i in range(5):
        ...
        print("batch_idx_ran= ", batch_idx_ran)
        ell = 5
...
for epoch in range(epochs):
    ...
    round_process1 = [i for i in range(1, len(acc_train_collect) + 1)]
    if round_process % 10 == 0:
        print("round_process= ", round_process)
        df = DataFrame({'round': round_process1, 'acc_train': acc_train_collect, 'acc_test': acc_test_collect,
...
round_process2 = [i for i in range(1, len(acc_train_collect) + 1)]
df = DataFrame({'round': round_process2, 'acc_train': acc_train_collect, 'acc_test': acc_test_collect,'loss_train': loss_train_collect,'loss_test': loss_test_collect})
file_name = program1 + ".xlsx"
df.to_excel(file_name, sheet_name="v1_test", index=False)
